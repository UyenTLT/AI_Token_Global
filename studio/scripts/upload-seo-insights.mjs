...
async function buildGsc() {
  ...
  return {
    _id: 'seoInsights-gsc',
    _type: 'seoInsights',
    title: 'Search Console — last ' + ov.meta.rangeDays + ' days',
    source: 'gsc',
    siteUrl: ov.meta.siteUrl,
    snapshotDate: ov.meta.snapshotDate,
    rangeDays: ov.meta.rangeDays,
    summary:
      `Google Search Console, last ${ov.meta.rangeDays} days for ${ov.meta.siteUrl}: ` +
...
async function buildCloudflare() {
  ...
  return {
    ...
    summary:
      ...
      `${num(c.visits)} visits (${change(c.visits, p.visits)} vs prior window), ` +
      `${num(c.pageViews)} page views` +
      `${c.medianLoadMs > 0 ? `, ${ms(c.medianLoadMs)} median load time` : ''}.`,
    metrics: keyed([
      ...
      { label: 'Median load time', value: c.medianLoadMs > 0 ? ms(c.medianLoadMs) : '—', change: change(c.medianLoadMs, p.medianLoadMs) },
    ]),
    topItems: keyed(top),
  };
}
...
async function main() {
  ...
  const stamped = docs.map((d) => ({ ...d, updatedAt: new Date().toISOString() }));

  if (!stamped.length) {
    console.log('No SEO snapshots found under seo-data/ — nothing to upload.');
    return;
  }

  console.log(`Built ${stamped.length} seoInsights doc(s): ${stamped.map((d) => d._id).join(', ')}`);
  if (DRY_RUN) {
    console.log(JSON.stringify(stamped, null, 2));
    return;
  }

  const ndjson = stamped.map((d) => JSON.stringify(d)).join('\n') + '\n';
  const file = resolve(STUDIO, '_seo-insights.ndjson');
  await writeFile(file, ndjson);
  try {
    execSync(`npx sanity dataset import "${file}" --dataset ${DATASET} --replace`, { cwd: STUDIO, stdio: 'inherit' });
    console.log('✓ seoInsights documents uploaded.');
  } finally {
